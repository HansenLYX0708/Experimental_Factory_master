...
def preprocess_single_image_RGB(image, save_path):
    # get image
    # convert to Gray
    image_gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)

    # gaussian filter
    gaussian_image = cv2.GaussianBlur(src=image_gray, ksize=(3, 3), sigmaX=1.5)
    #plt.subplot(131), plt.imshow(gaussian_image, "gray")
...
def preprocess_single_image_RGB_withoutAngle(image, save_path):
    # get image
    # convert to Gray
    image_gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
...
if __name__ == '__main__':
    ...
    if single:
        image_path = "okanoimage/AIA-58B0C3DA24.bmp"
        save_path = "AIA-58B0C3DA24.bmp"
        image = cv2.imread(image_path, 1)
        preprocess_single_image_RGB_withoutAngle(image, save_path)
    else:
